...
| ADR | Decision | Status |
| --- | --- | --- |
| ADR-001 | Branch-aware multi-tenancy via row-level `organisation_id`/`branch_id` (shared DB, shared schema); **design-ratified Gate 0A** 2026-06-28 ([#54](https://gitlab.com/cricketaustin-group/Pzure/-/issues/54)) against `Unified_ERD.md` (partition key `branch_id`, per-entity consistency). **Isolation enforcement (DB RLS + CI tests) pending [#70](https://gitlab.com/cricketaustin-group/Pzure/-/issues/70); not enforced until #70 is green on `develop`.** | Approved (design-ratified; enforcement pending #70) |
