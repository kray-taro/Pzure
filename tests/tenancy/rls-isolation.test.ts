...
run('ADR-001 §6.4 RLS isolation (live SQL Server)', () => {
  ...
  beforeAll(async () => {
    db = await connect();
    // Fixture schema mirrors the two scoping classes from ADR-001 §6.1:
    //   t_direct      -> owns branch_id (like emr_visits/billing_sales)
    //   t_child       -> inheritance-scoped via parent_id -> t_direct.id
    ...
    await db.raw(`
      ...
      INSERT INTO dbo.t_master (id, organisation_id, label) VALUES (NEWID(), NEWID(), 'shared-ref');
    `);
